...
const BASE_URL = 'http://localhost:5000';  // Это адрес вашего Express-сервера

export const getTopHeadLines = async (page = 1, pageSize = 10) => {
    const response = await axios.get(
      `${BASE_URL}/top-headlines?page=${page}&pageSize=${pageSize}`
    );
...
export const getSearchNews = async (searchQuery, page = 1, pageSize = 10) => {
    const response = await axios.get(
      `${BASE_URL}/search-news?searchQuery=${searchQuery}&page=${page}&pageSize=${pageSize}`
    );
    return response.data;
};
...
export const getCategoryNews = async (category, page = 1, pageSize = 10) => {
    const response = await axios.get(
      `${BASE_URL}/category-news?category=${category}&page=${page}&pageSize=${pageSize}`
    );
    return response.data;
};
